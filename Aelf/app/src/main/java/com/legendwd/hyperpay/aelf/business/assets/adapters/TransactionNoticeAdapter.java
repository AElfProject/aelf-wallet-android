...
public class TransactionNoticeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int TYPE_EMPTY = 1;
    private static final int TYPE_RECORD = 2;
    private Activity mActivity;
    private LayoutInflater layoutInflater;
    private NoticeRecordAdapter noticeRecordAdapter;
    private OnItemClickListener mClickListener;
    private List<TransactionNoticeBean.NoticeBean> mDataList = new ArrayList<>();


    public TransactionNoticeAdapter(Activity mActivity, List<TransactionNoticeBean.NoticeBean> list, OnItemClickListener clickListener) {
        this.mActivity = mActivity;
        layoutInflater = LayoutInflater.from(mActivity);
        this.mClickListener = clickListener;
        this.mDataList = list;
        noticeRecordAdapter = new NoticeRecordAdapter(mActivity, mDataList);
    }

    public void refreshData(List<TransactionNoticeBean.NoticeBean> list) {
        if (null == list)
            mDataList.clear();
        else
            mDataList = list;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof EmptyHolder) {
            ...
            ((NoticeRecordViewHolder) holder).recyclerView.setAdapter(noticeRecordAdapter);
            noticeRecordAdapter.setOnItemClickListener(mClickListener);
        }
